 ...
 */
export function fivePointStar(ctx: CanvasRenderingContext2D, x = 0, y = 0, maxr = 0, minr = 0, angle = 0) {
    if (!ctx) return;
    ctx.beginPath();
    for (let i = 0; i < 5; i++) {
...
export function ellipse(ctx: CanvasRenderingContext2D, sx :number, sy: number, ex: number, ey: number) {
